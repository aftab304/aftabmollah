  ...
  return (
    <section id="experience" ref={ref} className="mx-auto max-w-5xl px-6 pt-8 pb-20 scroll-mt-24">
      <SectionHeader
        ...
        title={
          <>
            Research, teaching & <span className="text-gradient italic">leadership</span>.
          </>
        }
        ...
        <div className="space-y-10">
          {items.map((it, i) => {
            ...
            return (
              ...
              >
                <div
                  className={`absolute left-4 md:left-1/2 top-3 md:-translate-x-1/2 h-5 w-5 rounded-full bg-[var(--azure)] ring-4 ring-background shadow-glow grid place-items-center`}
                >
                  <span className="h-1.5 w-1.5 rounded-full bg-background" />
                </div>
                <div className={`${right ? "md:col-start-2 md:pl-10" : "md:pr-10 md:text-right"}`}>
                  <div
                    className={`inline-flex items-center gap-2 glass rounded-full px-3 py-1 text-[10px] uppercase tracking-[0.18em] text-muted-foreground`}
                  >
